...
namespace Cidonix.UniBridge.MCP.Editor.ToolRegistry
{
    ...
    public static class McpToolRegistry
    {
        ...
        // Some MCP clients prefix tool names with "mcp_{serverName}_".
        // The MCP spec enforces max 64 chars total. Keep names compact, but allow
        // the longest curated UniBridge tool name to remain human-readable.
        const int k_MaxToolNameLength = 43;
        ...
        /// <summary>
        /// Executes a registered tool directly from another tool that already owns the relevant scheduler lease.
        /// External MCP requests should always use <see cref="ExecuteToolAsync"/>.
        /// </summary>
        ...
        static async Task<object> ExecuteToolCoreAsync(string toolName, JObject parameters, bool useScheduler)
        {
            ...
            try
            {
                var policy = ToolExecutionScheduler.ResolvePolicy(toolName, parameters, handler);
                ProjectContextGuard.AssertExpectedRootForPolicy(toolName, parameters, policy);
