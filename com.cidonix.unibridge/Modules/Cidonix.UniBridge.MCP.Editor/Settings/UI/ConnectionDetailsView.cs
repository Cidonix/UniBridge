...
namespace Cidonix.UniBridge.MCP.Editor.Settings.UI
{
    /// <summary>
    ...
    class ConnectionDetailsView : VisualElement
    {
        ...
        void InitializeElements()
        {
            ...
            var trustWarningContainer = this.Q<VisualElement>("trustWarningContainer");
            m_TrustWarning = new HelpBox("", HelpBoxMessageType.Warning);
            trustWarningContainer.Add(m_TrustWarning);
        ...
        public void SetConnectionInfo(ConnectionInfo info, ValidationDecision decision = null)
        {
            ...
            // Determine connection trust level
            var trustLevel = ConnectionTrustClassifier.DetermineTrustLevel(info);

            // Set trust warning
            SetTrustWarning(trustLevel, info);
        ...
        void SetTrustWarning(ConnectionTrustLevel trustLevel, ConnectionInfo info)
        {
            ...
            switch (trustLevel)
            {
                case ConnectionTrustLevel.Unknown:
                    m_TrustWarning.messageType = HelpBoxMessageType.Error;
                    m_TrustWarning.text = $"{appName} has connected. This application is unsigned or not recognized and may be dangerous!";
                    break;
                case ConnectionTrustLevel.Untrusted:
                    m_TrustWarning.messageType = HelpBoxMessageType.Warning;
                    m_TrustWarning.text = $"{appName} has connected. This application is unsigned - proceed with caution.";
                    break;
                case ConnectionTrustLevel.Trusted:
                    m_TrustWarning.messageType = HelpBoxMessageType.Info;
                    if (!string.IsNullOrWhiteSpace(publisherName))
                        m_TrustWarning.text = $"{appName} from {publisherName} is connected.";
                    else if (isSigned)
                        m_TrustWarning.text = $"{appName} (unnamed publisher) is connected.";
                    else
                        m_TrustWarning.text = $"{appName} is connected.";
                    break;
                default:
                    m_TrustWarning.messageType = HelpBoxMessageType.Warning;
                    m_TrustWarning.text = $"{appName} is connected to Unity.";
                    break;
            }
        }
