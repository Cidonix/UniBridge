...
using Cidonix.UniBridge.MCP.Editor.Settings.UI;
...
namespace Cidonix.UniBridge.MCP.Editor.Settings
{
    ...
    {
        const string k_McpServerDisclaimerAcceptedKey = "Cidonix.UniBridge.McpServerDisclaimerAccepted";
        ...
        Toggle m_AutoApproveEditorToggle;
        ...
        void RefreshDisclaimerState()
        {
            ...
            var accepted = EditorPrefs.GetBool(k_McpServerDisclaimerAcceptedKey, false);
            m_DisclaimerSection.style.display = accepted ? DisplayStyle.None : DisplayStyle.Flex;

            if (!accepted)
            {
                m_DisclaimerAcceptButton.style.display = DisplayStyle.Flex;
                m_DisclaimerSignInLabel.style.display = DisplayStyle.None;
            }
        ...
        {
            EditorPrefs.SetBool(k_McpServerDisclaimerAcceptedKey, true);
        ...
        void LoadUI()
        {
            ...
            {
                var fallbackLabel = new Label("UniBridge MCP Settings - UI template not found");
        ...
        void InitializeUI()
        {
            ...
            m_AutoApproveEditorToggle = m_RootElement.Q<Toggle>("autoApproveEditorToggle");
            m_AutoApproveEditorToggle.value = settings.autoApproveInEditorMode;
            m_AutoApproveEditorToggle.RegisterValueChangedCallback(evt => {
                settings.autoApproveInEditorMode = evt.newValue;
        ...
        void SetupConnectionsList()
        {
            ...
            var hasAnyConnectedClients = connectedClients.Count > 0;
        ...
        void UpdateConnectionPolicyLabel()
        {
            ...
            m_ConnectionPolicyLabel.text = maxDirect < 0
                ? "Unlimited local MCP connections allowed."
                : maxDirect == 1
                    ? "1 local MCP connection allowed at a time."
                    : $"Up to {maxDirect} local MCP connections allowed at a time.";
        }
