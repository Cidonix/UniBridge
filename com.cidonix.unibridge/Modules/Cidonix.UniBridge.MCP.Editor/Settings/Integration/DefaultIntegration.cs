...
namespace Cidonix.UniBridge.MCP.Editor.Settings.Integration
{
    ...
    /// Manages UniBridge MCP server configuration in client-specific config files.
    /// </summary>
    class DefaultIntegration : IClientIntegration
    {
        ...
        /// <summary>
        /// Configures the MCP client by adding UniBridge MCP server to its configuration file.
        /// </summary>
        ...
        /// <summary>
        /// Disables the integration by removing UniBridge MCP server from the client's configuration.
        /// </summary>
        ...
        public bool Disable()
        {
            ...
            try
            {
                ...
                var serverKey = McpServerKeyUtility.GetProjectServerKey();

                // Check if mcpServers exists and contains this project's entry
                if (existingConfig[Client.serversJsonKey] != null)
                {
                    ...
                    var removed = false;
                    if (mcpServers[serverKey] != null)
                    {
                        mcpServers.Remove(serverKey);
                        removed = true;
                    }

                    if (mcpServers[McpServerKeyUtility.LegacyUniBridgeKey] != null)
                    {
                        mcpServers.Remove(McpServerKeyUtility.LegacyUniBridgeKey);
                        removed = true;
                    }
                    ...
                    if (removed)
                    {
                        if (!mcpServers.HasValues)
                            existingConfig.Remove(Client.serversJsonKey);
                ...
                UpdateStatus(McpStatus.NotConfigured, "UniBridge MCP entry not found in configuration");
                return true;
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Checks whether the MCP client is properly configured with UniBridge MCP server.
      ...
      ""env"": {{}}
        ...
        ConfigResult UpdateExistingConfig(string configPath, string newConfig)
        {
            try
            {
                ...
                string serverKey = McpServerKeyUtility.GetProjectServerKey();

                // Merge this project's UniBridge entry into the existing servers.
                mcpServers[serverKey] = newMcpServers[serverKey];
                if (mcpServers[McpServerKeyUtility.LegacyUniBridgeKey] != null)
                    mcpServers.Remove(McpServerKeyUtility.LegacyUniBridgeKey);
        ...
        static bool ValidateConfigFile(string configPath, string serversKey)
        {
            try
            {
                ...
                string serverKey = McpServerKeyUtility.GetProjectServerKey();
                return config?[serversKey]?[serverKey] != null;
            }
            catch
            {
                return false;
