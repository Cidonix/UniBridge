...
namespace Cidonix.UniBridge.MCP.Editor.Security
{
    ...
    static class ExecutableIdentityCollector
    {
        ...
        const uint k_WtdUiNone = 2;
        const uint k_WtdRevokeNone = 0;
        const uint k_WtdChoiceFile = 1;
        const uint k_WtdStateActionVerify = 1;
        const uint k_WtdStateActionClose = 2;
        const uint k_WtdSaferFlag = 0x100;
        ...
        private static void CollectWindowsSignature(string executablePath, ExecutableIdentity identity)
        {
            try
            {
                ...
                using (cert)
                {
                    ...
                    identity.SignatureValid = VerifyWindowsAuthenticode(executablePath);
                }
            }
            catch (CryptographicException ex) when (IsUnsignedWindowsExecutable(ex))
            {
                identity.IsSigned = false;
                identity.SignatureValid = false;
            }
        ...
        static bool IsUnsignedWindowsExecutable(CryptographicException ex)
        {
            var message = ex.Message ?? string.Empty;
            return message.Contains("Unable to decode certificate", StringComparison.OrdinalIgnoreCase) ||
                   message.Contains("Cannot find the requested object", StringComparison.OrdinalIgnoreCase) ||
                   message.Contains("ASN1", StringComparison.OrdinalIgnoreCase);
        }

        static bool VerifyWindowsAuthenticode(string executablePath)
        {
            var fileInfo = new WINTRUST_FILE_INFO
            {
                cbStruct = (uint)Marshal.SizeOf<WINTRUST_FILE_INFO>(),
                pcwszFilePath = executablePath,
                hFile = IntPtr.Zero,
            ...
            var trustData = new WINTRUST_DATA
            {
                ...
                dwUIChoice = k_WtdUiNone,
                fdwRevocationChecks = k_WtdRevokeNone,
                dwUnionChoice = k_WtdChoiceFile,
                dwStateAction = k_WtdStateActionVerify,
                hWVTStateData = IntPtr.Zero,
                pwszURLReference = IntPtr.Zero,
                dwProvFlags = k_WtdSaferFlag,
                dwUIContext = 0,
            ...
            {
                trustData.dwStateAction = k_WtdStateActionClose;
