...
namespace Cidonix.UniBridge.MCP.Editor.Helpers
{
    ...
    static class ResourceUriHelper
    {
        ...
        /// </summary>
        /// <returns>The current UniBridge MCP script tools specification JSON string</returns>
            ...
            try
            {
                ...
                var availableTools = ToolRegistry.McpToolRegistry.GetAvailableTools(ignoreEnabledState: true)
                    .Where(t => t.name != null && (
                        ...
                        t.name == "UniBridge_ApplyTextEdits" ||
                        t.name == "UniBridge_ValidateScript" ||
                        t.name == "UniBridge_CreateScript" ||
                        t.name == "UniBridge_DeleteScript" ||
                        t.name == "UniBridge_GetSha"
                    ))
                    .OrderBy(t => t.name)
                    .ToArray();
      ...
      ""title"": ""{EscapeJsonString(tool.title ?? tool.name)}""
                ...
                var specJson = $@"{{
  ""name"": ""UniBridge MCP - Available Script Tools"",
  ""version"": ""1.0"",
  ""generated"": ""{DateTime.UtcNow:yyyy-MM-ddTHH:mm:ssZ}"",
  ""description"": ""Dynamically generated specification of available UniBridge script editing and management tools."",

  ""primary_tools"": {{
    ""script_reading"": ""UniBridge_ReadResource (via unity://path/Assets/... URIs)"",
    ""text_editing"": ""UniBridge_ApplyTextEdits (precise range-based edits with SHA validation)"",
    ""script_management"": ""UniBridge_Script (multi-action script router)"",
    ""validation"": ""UniBridge_ValidateScript (C# syntax and structure validation)"",
    ""file_operations"": ""UniBridge_CreateScript, UniBridge_DeleteScript""
  }},
  ...
  ""recommended_workflows"": [
    {{
      ...
      ""steps"": [
        ""1. UniBridge_ReadResource to get current content and SHA"",
        ""2. UniBridge_ValidateScript to check current state"",
        ""3. UniBridge_ApplyTextEdits with precondition_sha256"",
        ""4. UniBridge_ValidateScript to verify changes""
      ]
    }},
    {{
      ""name"": ""Create New Script"",
      ""steps"": [
        ""1. UniBridge_CreateScript with initial content"",
        ""2. UniBridge_ValidateScript to check syntax"",
        ""3. UniBridge_ApplyTextEdits for any corrections""
      ]
    }}
  ],
          ...
          ""args"": {{
            ...
            ""edits"": [
              {{
                ...
                ""endLine"": 10,
                ""endCol"": 1,
                ""newText"": ""    // Added by UniBridge MCP\\n""
              }}
            ],
  ...
  ""notes"": [
    ...
    ""Use UniBridge_ApplyTextEdits for precise range-based editing with 1-indexed line/column numbers."",
    ""Always use precondition_sha256 to prevent concurrent edit conflicts."",
    ""The UniBridge_Script tool is a multi-action script router; prefer specialized tools for precise reads and edits."",
    ""All script paths must be under Assets/ directory for security."",
    ""Use UniBridge_ValidateScript after edits to ensure C# syntax correctness.""
  ]
}}";
            ...
            catch (Exception ex)
            {
                ...
                return @"{
  ""name"": ""UniBridge MCP - Script Tools (Fallback)"",
  ...
  ""note"": ""Use UniBridge_ReadResource for reading files, UniBridge_ApplyTextEdits for editing, UniBridge_ValidateScript for validation.""
}";
            }
        }
