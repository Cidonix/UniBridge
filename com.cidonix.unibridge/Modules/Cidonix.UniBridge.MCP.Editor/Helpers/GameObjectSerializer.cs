    ...
    public static class GameObjectSerializer
    {
        ...
        public static object GetGameObjectData(GameObject go)
        {
            ...
            return new
            {
                ...
                staticEditorFlags = GameObjectUtility.GetStaticEditorFlags(go).ToString(),
        ...
        {
            // --- Add Early Logging ---
            // Debug.Log($"[GetComponentData] Starting for component: {c?.GetType()?.FullName ?? "null"} (ID: {UnityApiAdapter.GetObjectId(c)})");
            // --- End Early Logging ---
        ...
        /// <summary>
        /// Creates a shallow, JSON-safe component summary for MCP responses.
        /// This deliberately avoids arbitrary reflection over Unity components because some
        /// engine types expose properties that make Newtonsoft walk Matrix4x4/TRS internals
        /// and can lock the editor while serializing a tool response.
        /// </summary>
        public static object GetComponentSummaryData(Component c)
        {
            if (c == null)
            {
                return new Dictionary<string, object>
            ...
            if (c is Renderer renderer)
            {
                ...
                data["sortingLayerName"] = renderer.sortingLayerName;
                data["sortingOrder"] = renderer.sortingOrder;
                data["renderingLayerMask"] = RenderingLayerUtility.SerializeRenderingLayerMask(renderer);
            }

            if (c is Transform tr)
            {
                data["position"] = new { x = tr.position.x, y = tr.position.y, z = tr.position.z };
                data["localPosition"] = new { x = tr.localPosition.x, y = tr.localPosition.y, z = tr.localPosition.z };
                data["eulerAngles"] = new { x = tr.eulerAngles.x, y = tr.eulerAngles.y, z = tr.eulerAngles.z };
                data["localScale"] = new { x = tr.localScale.x, y = tr.localScale.y, z = tr.localScale.z };
                data["childCount"] = tr.childCount;
                data["parentName"] = tr.parent != null ? tr.parent.name : null;
            }

            if (c is Rigidbody2D body2D)
            {
                data["bodyType"] = body2D.bodyType.ToString();
                data["mass"] = body2D.mass;
                data["gravityScale"] = body2D.gravityScale;
                data["linearDamping"] = body2D.linearDamping;
                data["angularDamping"] = body2D.angularDamping;
                data["linearVelocity"] = new { x = body2D.linearVelocity.x, y = body2D.linearVelocity.y };
            }

            if (c is Collider2D collider2D)
            {
                data["enabled"] = collider2D.enabled;
                data["isTrigger"] = collider2D.isTrigger;
                UnityApiAdapter.AddCollider2DCompositeInfo(data, collider2D);
            }

            if (c is SpriteRenderer spriteRenderer)
            {
                data["spriteName"] = spriteRenderer.sprite != null ? spriteRenderer.sprite.name : null;
                data["color"] = new
                {
...
#if UNITY_EDITOR
            var scriptIdentity = ComponentIdentity.BuildScriptIdentity(c);
            if (scriptIdentity != null)
            {
                data["scriptIdentity"] = scriptIdentity;
                if (scriptIdentity.TryGetValue("monoScriptGuid", out var scriptGuid) && scriptGuid is string guid && !string.IsNullOrWhiteSpace(guid))
                {
                    data["scriptGuid"] = guid;
                }

                if (scriptIdentity.TryGetValue("serializedEditorClassIdentifier", out var serializedClassId) &&
                    serializedClassId is string classId &&
                    !string.IsNullOrWhiteSpace(classId))
                {
                    data["serializedEditorClassIdentifier"] = classId;
                }

                if (scriptIdentity.TryGetValue("namespaceMigration", out var migration) && migration != null)
                {
                    data["namespaceMigration"] = migration;
                }
            }
            ...
            if (c is MonoBehaviour monoBehaviour)
            {
                ...
                if (script != null)
                {
                    ...
                    data["scriptPath"] = AssetDatabase.GetAssetPath(script);
                }
            }
#endif
