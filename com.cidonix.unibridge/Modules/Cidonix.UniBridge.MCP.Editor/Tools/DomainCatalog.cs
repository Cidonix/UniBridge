        ...
        static object BuildDomainRiskControls(DomainDefinition domain, bool compact)
        {
            ...
            switch (domain.Key)
            {
                ...
                case "Assets":
                    ...
                    commonBefore.Add("Use SemanticDiff when comparing prefab/scene/material/.asset YAML deltas that are too noisy as raw text.");
                    commonExecute.Add("Use asset snapshots or BatchActions rollback for importer/material/data changes.");
                    commonVerify.Add("Inspect importer/asset context, run SemanticDiff for asset YAML changes, and refresh reference graph when asset deltas occurred.");
        ...
        static DomainDefinition[] GetDomains()
        {
            return new[]
            {
                ...
                new DomainDefinition
                {
                    ...
                    Notes = new[] { "WorkSession snapshots are stored in Library/UniBridge/WorkSessions and are not version-controlled.", "Executing BatchActions appends lightweight data.workSessionReview by default while a WorkSession is active; loaded-scene semantic review is skipped unless IncludeWorkSessionSemanticReview=true is explicit.", "Pass IncludeConsoleDelta=true and IncludeEditorEventDelta=true on BatchActions when the agent wants a compact self-check for logs and editor events emitted by that batch.", "ExecutionStatus Snapshot/Recent include the active WorkSession summary by default with bounded semantic review controls.", "Use Review and Diff before final reporting so the agent can explain exactly what changed.", "Revert defaults to DryRun=true and only executes when the agent explicitly passes DryRun=false." },
                ...
                new DomainDefinition
                {
                    ...
                    Notes = new[] { "Use AssetIntelligence Impact/ReferenceGraph before risky moves or deletes; pass IncludeReferenceLocations=true when exact YAML line/property/object context matters.", "Use AssetIntelligence SemanticDiff for compact review of Unity YAML/text asset deltas: document/class/fileID changes, property changes, GUID/script references, and line samples.", "Use AssetIntelligence Structure for compact prefab or loaded scene hierarchy list/search/read with indexed paths and serialized field matching.", "TypeSchema PatchExamples gives ready-to-call property patches for importers and ScriptableObjects.", "AssetSnapshotSerializer returns compact profiles for noisy materials, textures, clips, particles, TMP fonts, audio mixers, timelines, input actions, UI Toolkit assets, meshes, shaders, atlases, tiles, and related importer data." },
                    Aliases = new[] { "assets", "import", "importer", "materials", "sprites", "asset_structure", "prefab_structure", "semantic_asset_diff", "asset_semantic_diff", "unity_yaml_diff", "asset_ref_search", "reference_locations" }
                },
                new DomainDefinition
                {
                    Key = "Scripts",
