...
using Cidonix.UniBridge.MCP.Editor.Constants;
...
namespace Cidonix.UniBridge.MCP.Editor.Tools
{
    ...
    public static partial class ManageScript
    {
        ...
        /// </summary>
        public const string Title = "Route C# script operations";

        public const string Description = @"General C# script file operations for the Unity project.

Use this for simple create, read, delete, validate, get_sha, or routed edit operations. For new clients, prefer the specialized tools: UniBridge_ReadResource for reads, UniBridge_GetSha for hashes, UniBridge_CreateScript/DeleteScript for file lifecycle, UniBridge_ApplyTextEdits for exact range edits, and UniBridge_ScriptApplyEdits for structured method/class edits.

Args:
    action: create, read, update, delete, apply_text_edits, validate, edit, or get_sha.
    name: Script name without .cs extension.
    path: Folder under Assets/; defaults to Assets/Scripts when omitted.
    contents or encoded_contents: C# source for create/update operations.
...
Returns:
    success, message, and action-specific script data.";
        ...
        /// </summary>
        static bool TryResolveUnderAssets(string relDir, out string fullPathDir, out string relPathSafe)
        {
            string assets = Application.dataPath.Replace('\\', '/');

            // Normalize caller path: allow both "Scripts/..." and "Assets/Scripts/..."
            string rel = (relDir ?? "Scripts").Replace('\\', '/').Trim();
            // Strip "Assets/" prefix if present (fix for path doubling)
            ...
            {
                rel = ""; // Assets root - don't default to Scripts for this case
            }

            // Only default to "Scripts" if no directory was originally specified (null case)
            if (string.IsNullOrEmpty(rel) && relDir == null)
            {
                rel = "Scripts";
            }
            rel = rel.TrimStart('/');
            ...
            return new
            {
                ...
                properties = new
                {
                    action = new
                    {
                        ...
                        @enum = new[]
                        {
                            "create", "read", "update", "delete", "apply_text_edits",
                            "validate", "edit", "get_sha"
                        },
                        @default = "read"
                    },
                    name = new
                    ...
                    {
                        type = "string",
                        description = "Relative path under Assets/"
                    },
                    contents = new
                    {
                        type = "string",
                        description = "Script contents (plain text)"
                    },
                    contents_encoded = new
                    {
                        type = "boolean",
                    ...
                    {
                        type = "string",
                        description = "Type of script for templates/validation"
                    },
                    @namespace = new
                    {
                        type = "string",
                        description = "Namespace for organizing code"
                    },
                    ...
                    {
                        type = "array",
                        description = "List of edits. For apply_text_edits, each item should contain startLine,startCol,endLine,endCol,newText or a normalizable range.",
                        items = new
                        {
                            type = "object",
                            properties = new
                            {
                                ...
                                startCol = new { type = "integer" },
                                endLine = new { type = "integer" },
                                endCol = new { type = "integer" },
                                newText = new { type = "string" },
                                // Structured edit fields (for 'edit')
                                op = new { type = "string" },
                                ...
                                afterMethodName = new { type = "string" },
                                beforeMethodName = new { type = "string" }
                            }
                        }
                    },
                    precondition_sha256 = new { type = "string", description = "Optional SHA256 of current file to prevent concurrent edit conflicts" },
                    options = new
                    {
                        type = "object",
                        description = "Edit options (e.g., refresh/validate/applyMode)",
                        properties = new
                        {
                            refresh = new { type = "string", @enum = new[] { "immediate", "debounced", "none" } },
                            validate = new { type = "string", @enum = new[] { "basic", "standard", "strict", "comprehensive", "syntax", "relaxed" } },
                            applyMode = new { type = "string", @enum = new[] { "atomic", "sequential" } }
                        }
                    },

                    // validate
                    level = new { type = "string", description = "Validation level", @enum = new[] { "basic", "standard", "strict", "comprehensive" } }
                },
                required = new[] { "action", "name" }
            };
        }
        /// <summary>
        /// Main handler for multi-action script management requests.
        /// </summary>
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            switch (action)
            {
                ...
                case "read":
                    McpLog.Log("UniBridge_Script.read is supported, but UniBridge_ReadResource is preferred for direct file reads.");
                    return ReadScript(fullPath, relativePath);
                case "update":
                    McpLog.Log("UniBridge_Script.update is deprecated; prefer apply_text_edits. Serving update for backward compatibility.");
        ...
        /// </summary>
        const int MaxEditPayloadBytes = 64 * 1024;
            ...
            string validateMode = null)
        {
            ...
            if (spans.Count == 1)
            {
                ...
                var rx = new Regex(@"(?m)^[\t ]*(?:\[[^\]]+\][\t ]*)*[\t ]*(?:public|private|protected|internal|static|virtual|override|sealed|async|extern|unsafe|new|partial)[\s\S]*?\b([A-Za-z_][A-Za-z0-9_]*)\s*\(");
            ...
            try
            {
                ...
                if (immediate)
                {
                    McpLog.Log($"[ManageScript] ApplyTextEdits: immediate refresh for '{relativePath}'");
                ...
                {
                    McpLog.Log($"[ManageScript] ApplyTextEdits: debounced refresh scheduled for '{relativePath}'");
                    ManageScriptRefreshHelpers.ScheduleScriptRefresh(relativePath);
                }
        ...
        static bool TryIndexFromLineCol(string text, int line1, int col1, out int index)
        {
        ...
        static bool CheckScopedBalance(string text, int start, int end)
        {
        ...
        static object DeleteScript(string fullPath, string relativePath)
        {
            if (!File.Exists(fullPath))
            {
        ...
        static bool HasOverlaps(IEnumerable<(int start, int length, string text)> list)
        {
        ...
        static string ExtractReplacement(JObject op)
        {
        ...
        static string NormalizeNewlines(string t)
        {
            if (string.IsNullOrEmpty(t)) return t;
            return t.Replace("\r\n", "\n").Replace("\r", "\n");
        }

        static bool ValidateClassSnippet(string snippet, string expectedName, out string err)
        {
#if USE_ROSLYN
            try
            {
        ...
        static bool TryComputeClassSpan(string source, string className, string ns, out int start, out int length, out string why)
        {
#if USE_ROSLYN
            try
            {
        ...
        static bool TryComputeClassSpanBalanced(string source, string className, string ns, out int start, out int length, out string why)
        {
        ...
        static bool TryComputeMethodSpan(
            string source,
            int classStart,
            int classLength,
            string methodName,
        ...
        static int IndexOfTokenWithin(string s, string token, int start, int end)
        {
            int idx = s.IndexOf(token, start, StringComparison.Ordinal);
            return (idx >= 0 && idx < end) ? idx : -1;
        }

        static bool TryFindClassInsertionPoint(string source, int classStart, int classLength, string position, out int insertAt, out string why)
        {
            insertAt = 0; why = null;
            int searchStart = classStart;
            int searchEnd = Math.Min(source.Length, classStart + classLength);
        ...
        static int IndexOfClassToken(string s, string className)
        {
        ...
        static bool AppearsWithinNamespaceHeader(string s, int pos, string ns)
        {
        ...
        /// </summary>
        static string GenerateDefaultScriptContent(
            string name,
            string scriptType,
            string namespaceName
        )
        ...
        // Script validation internals live in ManageScript.Validation.cs.
