...
namespace Cidonix.UniBridge.MCP.Editor.Tools
{
    ...
    {
        /// <summary>
        /// Human-readable description of the UniBridge_GetSha tool functionality and usage.
        /// </summary>
        public const string Title = "Get file SHA256 hash";

        public const string Description = @"Get SHA256 and basic metadata for a C# script without reading its contents.

Use this before applying edits when you need a precondition_sha256 guard against concurrent file changes.

Args:
    uri: unity://path/..., file://..., or Assets/... path to a .cs file.

Returns:
    success, message, and data with sha256, lengthBytes, uri, path, and lastModifiedUtc.";
        ...
        [McpOutputSchema("UniBridge_GetSha")]
        ...
        [McpTool("UniBridge_GetSha", Description, Title, Groups = new string[] { "core", "scripting" }, EnabledByDefault = true)]
        public static object HandleCommand(GetSHAParams parameters)
        {
            ...
            {
                return Response.Error($"UniBridge_GetSha_failed, UniBridge_GetSha error: {e.Message}");
            }
        }
    }
}
