...
using Cidonix.UniBridge.MCP.Editor.Helpers; // For Response class
...
namespace Cidonix.UniBridge.MCP.Editor.Tools
{
    ...
    {
        /// <summary>
        /// Description of the UniBridge_ManageScene tool for MCP clients.
        ...
        public const string Description = @"Create, load, save, inspect, and query Unity scenes.

Use this for active scene information, hierarchy inspection, build settings, and basic scene file operations.

Args:
    Action: Create, Load, Save, GetHierarchy, GetActive, or GetBuildSettings.
    Name: Scene name without .unity for Create, Load, or Save.
    Path: Scene asset path or folder under Assets/ for scene files; Create defaults to Assets/Scenes.
    BuildIndex: Build index for build-settings-related lookups.
    Depth: Hierarchy depth for GetHierarchy (-1 full, 0 roots only, 1+ limited).

Returns:
    success, message, and scene data such as path, build index, dirty/loaded state, root count, or hierarchy.";
        /// <summary>
        /// Returns the output schema for this tool.
        /// </summary>
        /// <returns>The JSON schema object describing the tool's output structure.</returns>
        [McpOutputSchema("UniBridge_ManageScene")]
        ...
        [McpTool("UniBridge_ManageScene", Description, Title, Groups = new string[] { "core", "scene" }, EnabledByDefault = true)]
        public static object HandleCommand(ManageSceneParams parameters)
        {
            var @params = parameters;

            string name = @params.Name;
            string path = @params.Path; // Scene asset path or folder relative to Assets/
            int? buildIndex = @params.BuildIndex;

            string normalizedPath = NormalizeProjectPathCandidate(path);
            string explicitScenePath = !string.IsNullOrWhiteSpace(normalizedPath) &&
                normalizedPath.EndsWith(".unity", StringComparison.OrdinalIgnoreCase)
                    ? NormalizeSceneAssetPath(normalizedPath)
                    : null;

            if (!string.IsNullOrEmpty(explicitScenePath) && string.IsNullOrEmpty(name))
            {
                name = Path.GetFileNameWithoutExtension(explicitScenePath);
            }

            string sceneFileName = string.IsNullOrEmpty(name) ? null : $"{name}.unity";
            string relativePath = explicitScenePath;
            ...
            {
                // Ensure folder paths are relative to Assets/, removing any leading "Assets/".
                string relativeDir = normalizedPath ?? string.Empty;
                ...
                if (string.IsNullOrEmpty(path) && @params.Action == SceneAction.Create)
                {
                    relativeDir = "Scenes";
                }
            ...
            switch (@params.Action)
            {
                case SceneAction.Create:
                    if (string.IsNullOrEmpty(name) || string.IsNullOrEmpty(relativePath))
                        return Response.Error(
                            "'name' and 'path' parameters are required for 'create' action, or pass Path as an Assets/... .unity scene path."
                        );
        ...
        static string NormalizeSceneAssetPath(string path)
        {
        ...
        static object GetGameObjectDataRecursive(GameObject go, int depth = -1)
        {
            ...
            var gameObjectData = new Dictionary<string, object>
            {
                ...
                { "instanceID", UnityApiAdapter.GetObjectId(go) }, // Useful unique identifier
