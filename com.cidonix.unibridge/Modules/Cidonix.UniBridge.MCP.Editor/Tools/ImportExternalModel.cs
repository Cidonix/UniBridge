...
namespace Cidonix.UniBridge.MCP.Editor.Tools
{
    ...
    {
        /// <summary>
        /// Human-readable description of the UniBridge_ImportExternalModel tool functionality and usage.
        /// </summary>
        public const string Title = "Import an external 3D model";

        public const string Description = @"Import an external FBX model into the Unity project and current scene.

Use this when a model file is available as a local path, URL, or zip containing an FBX. The tool imports the model under Assets/ExternalModels, creates a scene instance, and saves a prefab for reuse.

Args:
    name: Simple asset identifier without spaces.
    fbx_url: Local file path, URL, or zip path/URL containing an FBX.
    height: Desired world-space height for the imported scene instance.
    albedo_texture_url: Optional local path or URL for an albedo texture.

Returns:
    success, message, and data with importDirectory, sceneObject, prefabObject, and prefabPath. Use the returned bounds/center data before positioning the object relative to other scene objects.";
        ...
        [McpOutputSchema("UniBridge_ImportExternalModel")]
        ...
        [McpTool("UniBridge_ImportExternalModel", Description, Title, Groups = new string[] { "core", "assets" }, EnabledByDefault = true)]
        ...
        static void ValidateFilesURL(string fbxUrl, string albedoTextureURL)
        {
            ...
            {
                throw new Exception($"Fbx is already in the project, use UniBridge_ManageAsset tool to manage asset that are already in the project");
            }
            if (hasTextureURL && !IsHttpString(albedoTextureURL) && IsInProject(albedoTextureURL))
            {
                throw new Exception($"Albedo texture is already in the project, use UniBridge_ManageAsset tool to manage asset that are already in the project");
            }
        }
